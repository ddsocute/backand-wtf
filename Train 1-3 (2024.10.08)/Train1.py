...
def load_txt(txt_path):
    ...
    for data_line_str in loaded_txt:
        data_line_list = []
        for each_student in data_line_str.split():
            if each_student.isdigit():  # 先判斷是不是數字是的話先轉換再寫入
                each_student = float(each_student)
            data_line_list.append(each_student)
        data_full_list_local.append(data_line_list)
    for each_person in data_full_list_local:  # 用全列表人數去跑迴圈
        each_person_total = 0
