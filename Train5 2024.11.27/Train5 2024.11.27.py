    ...
    def display_student(
        ...
        for student in student_be_displayed:
            ...
            for attribute in display_attribute:
                value = getattr(
                    student, attribute
                )  # 取得該學生該屬性值 TODO:用key值判斷會更好 不然註解要寫清楚 巢狀結構超過三層要重新構思 太多了
                if isinstance(value, list):  # 篩出至少一層列表的
                    if isinstance(value[0], list):  # 篩出兩層列表的 exam_history
                        display_gradelist = value[times - 1]
                        for display_grade in display_gradelist:
                            display_row.append(f"{display_grade:.2f}")  # 逐個加入
                    else:
                        display_row.append(
                            f"{value[times - 1]:.2f}"
                        )  # 加入 total average
...
if __name__ == "__main__":
    ...
    student_management = StudentManagement()  # TODO:不用new
    header_list = [
        HEADER,
    ...
    display_object_list = [
        lambda: student_management.get_student_in_exam(students, exam_time),  # TODO: 認知成本變高 多function call成本 
        lambda: student_management.get_subject(
            student_management.get_student_in_exam(students, exam_time), exam_time
        ),
        lambda: StudentManagement.get_student_rank(
            student_management.get_student_in_exam(students, exam_time), exam_time
        ),
        lambda: StudentManagement.get_find_student(
            student_management.get_student_in_exam(students, exam_time)
        ),
    ]
    display_attribute_list = [
        list(vars(students[0]).keys()),
        None,
        ["name", "average"],
        list(vars(students[0]).keys()),
    ]
    ...
    while True:
        ...
        if option == 0:
            ...
            if exam_time == 0:
                ...
                if option == 1:  # TODO:不要用列表存參數 直接放進去就好 可以用變數當中繼
                    StudentManagement.display_student(
                        exam_time,
                        header_list[0],
                        display_object_list[0](),
                        display_attribute_list[0],
                    )
                elif option == 2:
                    StudentManagement.display_subject(
                        header_list[1], display_object_list[1]()
                    )
                elif option == 3:
                    StudentManagement.display_student(
                        exam_time,
                        header_list[2],
                        display_object_list[2](),
                        display_attribute_list[2],
                        rank=True,
                    )
                elif option == 4:
                    StudentManagement.display_student(
                        exam_time,
                        header_list[3],
                        display_object_list[3](),
                        display_attribute_list[3],
                    )
